        ...
        {ok, Data1} ->
            ...
            if 
                Type == "register" ->
                    ...
                    if
                        Output == [] ->
                            ...
                            ok = gen_tcp:send(Socket, "Hi, User Registered"), % RESPOND BACK - YES/NO
                            io:fwrite("Let's Go!!\n");
                        true ->
                            ok = gen_tcp:send(Socket, "Sorry the Username is already used! Try Again:("),
                            io:fwrite("Oops, User is already in the database!\n")
                    end,
                ...
                Type == "subscribe" ->
                    ...
                    if
                        Output1 == [] ->
                            io:fwrite("Please Enter the User Name, The username entered doesn't exist!: \n");
                        true ->
                    ...
                    ok = gen_tcp:send(Socket, "Server processed the subscription!"),
                    do_recv(Socket, Table, [UserName], Client_Socket_Mapping);
                ...
                Type == "query" ->
                    ...
                    if
                        ...
                        Option == "2" ->
                            ...
                            ok = gen_tcp:send(Socket, Tweets);
                        true ->
                            io:fwrite("User Tweets that are Subscribed\n"),
...
searchUserTweets(Table, UserName) ->
    ...
    if
        Length == 0 ->
            io:fwrite("Sorry, User Not found!\n"),
            [];
        true ->
            Val3 = lists:nth(1, Val),
            Val2 = element(2, Val3),
...
sendMessage(Socket, Client_Socket_Mapping, Tweet, Subscribers, UserName) ->
    if
        ...
        true ->
            ...
            ok = gen_tcp:send(Client_Socket, ["You have Tweeted!\n",UserName,":",Tweet]),
            ok = gen_tcp:send(Socket, "Your tweet is sent!\n"),
